...
var _ = tlscertificate.RegisterInternalTLSHookEM(tlscertificate.GenSelfSignedTLSHookConf{
	...
	SANs: tlscertificate.DefaultSANs([]string{
		...
		"%CLUSTER_DOMAIN%://" + fmt.Sprintf("%s.%s.svc", settings.ControllerCertCN, settings.ModuleNamespace),
		fmt.Sprintf("%s-metrics", settings.ControllerCertCN),
		fmt.Sprintf("%s-metrics.%s", settings.ControllerCertCN, settings.ModuleNamespace),
		fmt.Sprintf("%s-metrics.%s.svc", settings.ControllerCertCN, settings.ModuleNamespace),
		"%CLUSTER_DOMAIN%://" + fmt.Sprintf("%s-metrics.%s.svc", settings.ControllerCertCN, settings.ModuleNamespace),
	}),
