...
func TestPoolPredicates(t *testing.T) {
	pred := poolPredicates()
	if !pred.Create(event.CreateEvent{Object: &v1alpha1.ClusterGPUPool{}}) {
		t.Fatalf("expected create to trigger")
	}

	old := &v1alpha1.ClusterGPUPool{Spec: v1alpha1.GPUPoolSpec{Resource: v1alpha1.GPUPoolResourceSpec{Unit: "Card"}}}
	...
	newDiff.Spec.Resource.MaxDevicesPerNode = ptrToInt32(2)
