...
func TestNodeMarkHandlePoolReturnsAltLabelListError(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = corev1.AddToScheme(scheme)
	_ = v1alpha1.AddToScheme(scheme)
	...
	base := testutil.WithNodeTaintIndexes(testutil.WithPoolDeviceIndexes(fake.NewClientBuilder().
		WithScheme(scheme))).
		Build()
	...
	if _, err := h.HandlePool(context.Background(), pool); err == nil {
		t.Fatalf("expected alt label list error")
	}
}

func TestNodeMarkHandlePoolReturnsAltTaintedListError(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = corev1.AddToScheme(scheme)
	_ = v1alpha1.AddToScheme(scheme)
	...
	h := NewNodeMarkHandler(testr.New(t), nodeMarkListInterceptClient{
		...
		fail: func(list client.ObjectList, opts []client.ListOption) error {
			...
			for _, opt := range opts {
				fields, ok := opt.(client.MatchingFields)
				if ok && fields[indexer.NodeTaintKeyField] == altKey {
					return errors.New("boom")
				}
			}
			return nil
	...
	if _, err := h.HandlePool(context.Background(), &v1alpha1.GPUPool{ObjectMeta: metav1.ObjectMeta{Name: "pool-a"}}); err == nil {
		t.Fatalf("expected alt tainted list error")
	}
}
...
func TestNodeMarkSyncNodeReturnsGetError(t *testing.T) {
	...
	if err := h.syncNode(context.Background(), "node1", "gpu.deckhouse.io/pool-a", "cluster.gpu.deckhouse.io/pool-a", true, false); err == nil {
...
func TestNodeMarkSyncNodeNoopWhenNodeMissing(t *testing.T) {
	...
	if err := h.syncNode(context.Background(), "missing", "gpu.deckhouse.io/pool-a", "cluster.gpu.deckhouse.io/pool-a", true, false); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestNodeMarkSyncNodeRemovesAlternateLabelAndTaintWhenNoDevices(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = corev1.AddToScheme(scheme)

	poolKey := "gpu.deckhouse.io/pool-a"
	altKey := "cluster.gpu.deckhouse.io/pool-a"
	value := "pool-a"

	node := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:   "node1",
			Labels: map[string]string{poolKey: value, altKey: value},
		},
	...
	h := NewNodeMarkHandler(testr.New(t), cl)
	if err := h.syncNode(context.Background(), "node1", poolKey, altKey, false, false); err != nil {
	...
	if _, ok := updated.Labels[altKey]; ok {
		t.Fatalf("expected alternate label to be removed, got %+v", updated.Labels)
	}
	if hasTaint(updated.Spec.Taints, altKey, corev1.TaintEffectNoSchedule) {
		t.Fatalf("expected alternate taint to be removed, got %+v", updated.Spec.Taints)
	}
}
