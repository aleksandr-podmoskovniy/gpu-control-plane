...
func TestDeviceNodeName(t *testing.T) {
	...
	if DeviceNodeName(dev) != "" {
		t.Fatalf("expected empty nodeName when labels are nil")
	}

	dev.Labels = map[string]string{deviceNodeLabelKey: " node-b "}
	if DeviceNodeName(dev) != "node-b" {
		t.Fatalf("expected nodeName from gpu label")
	}

	dev.Labels = map[string]string{"kubernetes.io/hostname": " node-c "}
	if DeviceNodeName(dev) != "node-c" {
		t.Fatalf("expected nodeName from hostname label")
	}
}
...
func TestAlternatePoolResourcePrefixFor(t *testing.T) {
	clusterPool := &v1alpha1.GPUPool{TypeMeta: metav1.TypeMeta{Kind: "ClusterGPUPool"}}
	if got := alternatePoolResourcePrefixFor(clusterPool); got != namespacedPoolResourcePrefix {
		t.Fatalf("unexpected prefix for cluster pool: %s", got)
	}

	nsPool := &v1alpha1.GPUPool{ObjectMeta: metav1.ObjectMeta{Namespace: "ns"}}
	if got := alternatePoolResourcePrefixFor(nsPool); got != clusterPoolResourcePrefix {
		t.Fatalf("unexpected prefix for namespaced pool: %s", got)
	}
}
