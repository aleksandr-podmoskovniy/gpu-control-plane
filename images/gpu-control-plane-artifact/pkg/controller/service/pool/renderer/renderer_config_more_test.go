...
func TestNewRendererHandlerUsesEnvFallbacks(t *testing.T) {
	...
	if h.cfg.Namespace != "env-ns" {
		t.Fatalf("unexpected namespace: %q", h.cfg.Namespace)
	}
	if h.cfg.DevicePluginImage != "dp:env" {
		t.Fatalf("unexpected device-plugin image: %q", h.cfg.DevicePluginImage)
	}
	if h.cfg.ValidatorImage != "val:env" {
		t.Fatalf("unexpected validator image: %q", h.cfg.ValidatorImage)
	}
}
