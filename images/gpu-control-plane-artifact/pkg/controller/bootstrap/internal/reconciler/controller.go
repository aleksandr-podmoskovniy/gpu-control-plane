...
func (r *Reconciler) SetupController(ctx context.Context, mgr manager.Manager, ctr controller.Controller) error {
	...
	for _, w := range []Watcher{
		watchers.NewGPUNodeStateModuleConfigWatcher(r.log.WithName("watcher.moduleconfig"), r.store),
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	inventory, err := commonobject.FetchObject(ctx, types.NamespacedName{Name: req.Name}, r.client, inventory)
	if err != nil {
	...
	rec.SetHandlerExecutor(func(ctx context.Context, handler Handler) (contracts.Result, error) {
