...
func TestSetupControllerBranches(t *testing.T) {
	...
	t.Run("propagates-new-error", func(t *testing.T) {
		...
		if err := SetupController(ctx, mgr, testr.New(t), config.ControllerConfig{}, store); err == nil {
	...
	t.Run("propagates-webhook-error", func(t *testing.T) {
		scheme := runtime.NewScheme()
		_ = corev1.AddToScheme(scheme) // omit v1alpha1 to make webhook registration fail
		cl := fake.NewClientBuilder().WithScheme(scheme).Build()
	...
	t.Run("success", func(t *testing.T) {
		...
		if err := SetupController(ctx, mgr, testr.New(t), cfg, nil); err != nil {
