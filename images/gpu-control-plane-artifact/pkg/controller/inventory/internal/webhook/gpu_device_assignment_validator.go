...
const (
	namespacedAssignmentAnnotation = commonannotations.GPUDeviceAssignment
	clusterAssignmentAnnotation    = commonannotations.ClusterGPUDeviceAssignment
)
...
func (v *GPUDeviceAssignmentValidator) validateAssignment(ctx context.Context, device *v1alpha1.GPUDevice, namespacedPool, clusterPool string) error {
	...
	if clusterPool != "" {
		...
		poolObj, err := commonobject.FetchObject(ctx, client.ObjectKey{Name: clusterPool}, v.client, poolObj)
		if err != nil {
			return err
		}
		if poolObj == nil {
