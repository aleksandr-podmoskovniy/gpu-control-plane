...
func TestDeviceApprovalAutoAttachAutomaticMode(t *testing.T) {
	policy, err := NewDeviceApprovalPolicy(moduleconfig.DeviceApprovalSettings{
		Mode: moduleconfig.DeviceApprovalModeAutomatic,
	})
	if err != nil {
	...
	if !policy.AutoAttach(true, labels.Set{}) {
		t.Fatal("automatic mode should auto attach when managed")
	}
}

func TestNewDeviceApprovalPolicySelectorWithNilSelectorMatchesEverything(t *testing.T) {
	policy, err := NewDeviceApprovalPolicy(moduleconfig.DeviceApprovalSettings{
		Mode:     moduleconfig.DeviceApprovalModeSelector,
		Selector: nil,
	})
	...
	if !policy.AutoAttach(true, labels.Set{}) {
		t.Fatal("empty selector should match everything in selector mode")
	}
}
