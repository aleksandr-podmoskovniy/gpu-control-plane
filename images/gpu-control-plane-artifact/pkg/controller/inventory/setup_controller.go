...
import (
	...
	ctrlwatchers "github.com/aleksandr-podmoskovniy/gpu-control-plane/controller/pkg/controller/watchers"
)
...
func (r *Reconciler) mapModuleConfigRequests(ctx context.Context, _ client.Client, moduleConfig *unstructured.Unstructured) ([]reconcile.Request, error) {
	return r.mapModuleConfig(ctx, moduleConfig), nil
}
...
func (r *Reconciler) SetupController(ctx context.Context, mgr manager.Manager, ctr controller.Controller) error {
	...
	for _, w := range []Watcher{
		...
		ctrlwatchers.NewModuleConfigWatcher(
			r.log.WithName("watcher.moduleconfig"),
			r.store,
			"",
			r.mapModuleConfigRequests,
		),
	} {
