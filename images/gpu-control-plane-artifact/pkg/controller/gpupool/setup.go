...
func SetupController(
	...
	if mgr.GetWebhookServer() != nil {
		...
		if err := builder.WebhookManagedBy(mgr).
			...
			WithValidator(NewGPUPoolValidator(baseLog, client, admissionHandlers)).
			WithDefaulter(NewGPUPoolDefaulter(baseLog, admissionHandlers)).
			Complete(); err != nil {
			return err
		}
		...
		if err := builder.WebhookManagedBy(mgr).
			For(&corev1.Pod{}).
			WithDefaulter(NewPodDefaulter(baseLog, store, client)).
			WithValidator(NewPodValidator(baseLog, client)).
			Complete(); err != nil {
			return err
		}
	}
