...
type AdmissionHandler interface {
	SyncPool(ctx context.Context, pool *v1alpha1.GPUPool) (reconcile.Result, error)
}
...
func NewGPUPoolValidator(log logr.Logger, c client.Client, handlers []AdmissionHandler) *GPUPoolValidator {
...
func NewGPUPoolDefaulter(log logr.Logger, handlers []AdmissionHandler) *GPUPoolDefaulter {
...
func validateNamespacedPoolNameUnique(ctx context.Context, c client.Client, pool *v1alpha1.GPUPool, admissionNamespace string) error {
	...
	clusterPool, err := commonobject.FetchObject(ctx, client.ObjectKey{Name: name}, c, clusterPool)
	if err != nil {
