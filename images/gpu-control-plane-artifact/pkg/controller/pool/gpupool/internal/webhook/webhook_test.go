...
func (h errorAdmissionHandler) SyncPool(context.Context, *v1alpha1.GPUPool) (reconcile.Result, error) {
...
func TestGPUPoolValidatorHandlerError(t *testing.T) {
	...
	cl := fake.NewClientBuilder().WithScheme(scheme).Build()
	validator := cradmission.WithCustomValidator(scheme, &v1alpha1.GPUPool{}, NewGPUPoolValidator(testr.New(t), cl, []AdmissionHandler{errHandler}))
	if resp := validator.Handle(context.Background(), req); resp.Allowed {
...
func TestGPUPoolDefaulterHandlerError(t *testing.T) {
	...
	defaulter := cradmission.WithCustomDefaulter(scheme, &v1alpha1.GPUPool{}, NewGPUPoolDefaulter(testr.New(t), []AdmissionHandler{errHandler}))
	resp := defaulter.Handle(context.Background(), req)
	if resp.Allowed {
