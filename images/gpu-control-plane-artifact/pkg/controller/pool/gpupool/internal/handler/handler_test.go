...
import (
	...
	"github.com/aleksandr-podmoskovniy/gpu-control-plane/controller/pkg/controller/pool/gpupool/internal/state"
)
...
func (s *stubPoolHandler) HandlePool(_ context.Context, pool *v1alpha1.GPUPool) (reconcile.Result, error) {
	s.calls++
	s.handled = pool
...
func TestWrapPoolHandlerDelegatesCalls(t *testing.T) {
	...
	target := &stubPoolHandler{name: "stub", result: reconcile.Result{Requeue: true}, err: expectedErr}
	...
	res, err := adapter.Handle(context.Background(), st)

	if err != expectedErr {
		t.Fatalf("expected error to be propagated")
	}
	if !res.Requeue {
		t.Fatalf("expected reconcile result to be propagated")
	}
