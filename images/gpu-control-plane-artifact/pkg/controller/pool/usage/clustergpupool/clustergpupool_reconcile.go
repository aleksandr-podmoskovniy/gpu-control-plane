...
import (
	...
	puinternal "github.com/aleksandr-podmoskovniy/gpu-control-plane/controller/pkg/controller/pool/usage/internal"
...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	s := pustate.NewClusterGPUPool(r.client, pool)

	rec := ctrlreconciler.NewBaseReconciler(r.handlers)
	rec.SetHandlerExecutor(func(ctx context.Context, handler puinternal.ClusterGPUPoolHandler) (reconcile.Result, error) {
	...
	rec.SetResourceUpdater(func(ctx context.Context) error {
		...
		return r.patchStatus(ctx, types.NamespacedName{Name: req.Name}, s.Used())
	})
