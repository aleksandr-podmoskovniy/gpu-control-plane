...
// NewConstructor returns controller-runtime LogConstructor that injects namespace/name attributes.
func NewConstructor(log *log.Logger) func(req *reconcile.Request) logr.Logger {
	return func(req *reconcile.Request) logr.Logger {
		logger := log
		if req != nil {
			logger = log.With(SlogNamespace(req.Namespace), SlogName(req.Name))
		}
