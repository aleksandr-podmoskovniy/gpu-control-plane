...
// SharingUnprepareStep reverts sharing settings like time-slicing and MPS.
type SharingUnprepareStep struct {
	timeSlicing ports.TimeSlicingManager
	mps         ports.MpsManager
}

// NewSharingUnprepareStep constructs a sharing unprepare step.
func NewSharingUnprepareStep(timeSlicing ports.TimeSlicingManager, mps ports.MpsManager) SharingUnprepareStep {
	return SharingUnprepareStep{timeSlicing: timeSlicing, mps: mps}
}

func (s SharingUnprepareStep) Take(ctx context.Context, st *state.UnprepareState) (*reconcile.Result, error) {
	if st == nil {
	...
	for _, dev := range st.Claim.Devices {
		...
		switch dev.Sharing.Strategy {
		case configapi.MpsStrategy:
			if dev.Sharing.MPS == nil {
				continue
			}
			mpsStates[dev.Sharing.MPS.ControlID] = *dev.Sharing.MPS
		case configapi.TimeSlicingStrategy:
