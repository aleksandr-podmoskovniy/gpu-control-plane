...
// RequestEnqueuer maps an object to reconcile requests.
...
// UpdateEventsFilter decides whether an update event should be processed.
type UpdateEventsFilter interface {
	FilterUpdateEvents(event.UpdateEvent) bool
}

// ObjectRefWatcher wires a watch with custom filtering and enqueueing.
type ObjectRefWatcher struct {
	filter   UpdateEventsFilter
	enqueuer RequestEnqueuer
}

// NewObjectRefWatcher creates a new ObjectRefWatcher.
func NewObjectRefWatcher(filter UpdateEventsFilter, enqueuer RequestEnqueuer) *ObjectRefWatcher {
...
// Run registers the watch in the controller.
