...
// Manager provides a compatibility wrapper around conditions.
// Deprecated: prefer direct SetCondition usage.
type Manager struct {
...
// Add adds a condition only if it does not already exist.
...
// Update updates or adds a condition.
func (m *Manager) Update(c metav1.Condition) {
	meta.SetStatusCondition(&m.conds, c)
}

// Generate returns the current list of conditions.
