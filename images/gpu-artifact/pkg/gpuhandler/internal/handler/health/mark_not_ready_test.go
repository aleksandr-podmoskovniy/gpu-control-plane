...
func TestMarkNotReadyHandler(t *testing.T) {
	...
	cond := meta.FindStatusCondition(updatedReady.Status.Conditions, handler.HardwareHealthyType)
	if cond != nil {
	...
	cond = meta.FindStatusCondition(updatedNotReady.Status.Conditions, handler.HardwareHealthyType)
