...
func TestCapabilitiesHandlerSuccess(t *testing.T) {
	...
	snapshot := &capabilities.DeviceSnapshot{
	...
	cond := meta.FindStatusCondition(updated.Status.Conditions, handler.HardwareHealthyType)
...
func TestCapabilitiesHandlerOpenFailure(t *testing.T) {
	...
	reader := &fakeReader{err: fmt.Errorf("init: %w", capabilities.ErrNVMLUnavailable)}
	tracker := &fakeTracker{shouldAttempt: true, recordFailure: true}
	h := NewCapabilitiesHandler(reader, service.NewPhysicalGPUService(client), tracker, nil)
...
func TestCapabilitiesHandlerMultiGPU(t *testing.T) {
	...
	session := &fakeSession{
		snapshots: map[string]*capabilities.DeviceSnapshot{
			"0000:02:00.0": snapshot,
		},
		errs: map[string]error{
			"0000:03:00.0": capabilities.ErrNVMLQueryFailed,
		},
	}
	reader := &fakeReader{session: session}
	tracker := &fakeTracker{shouldAttempt: true, recordFailure: true}
	...
	cond := meta.FindStatusCondition(updatedA.Status.Conditions, handler.HardwareHealthyType)
	if cond == nil || cond.Status != metav1.ConditionTrue {
	...
	cond = meta.FindStatusCondition(updatedB.Status.Conditions, handler.HardwareHealthyType)
...
type fakeReader struct {
	session capabilities.CapabilitiesSession
	err     error
}

func (f *fakeReader) Open() (capabilities.CapabilitiesSession, error) {
	if f.err != nil {
		return nil, f.err
	}
...
func (s *fakeSession) ReadDevice(pciAddress string) (*capabilities.DeviceSnapshot, error) {
	if s.err != nil {
		return nil, s.err
	}
	if s.errs != nil {
