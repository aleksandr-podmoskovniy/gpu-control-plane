...
func hardwareConditionChanged(prev, next *gpuv1alpha1.PhysicalGPU) bool {
	...
	oldCond := meta.FindStatusCondition(prev.Status.Conditions, hardwareHealthyType)
	newCond := meta.FindStatusCondition(next.Status.Conditions, hardwareHealthyType)
	if newCond == nil {
		return false
	}
	if oldCond == nil {
