...
function Help() {
   ...
   Syntax: relocate_binaries.sh [-h|f|i|o]
   options:

   -f     File with paths to binaries; wildcards supported.
   -i     Paths to binaries separated by space; wildcards supported.
          Example: -i "/bin/chmod /sbin/m*"
   -o     Output directory (default: /relocate)
