...
func configStoreWithScheduling(strategy, topologyKey string) *config.ModuleConfigStore {
...
func TestCollectPoolNodeTaintsClusterPrefix(t *testing.T) {
	...
	taints, err := m.collectPoolNodeTaints(context.Background(), pool)
	if err != nil {
...
func TestTopologyLabelPresentBranches(t *testing.T) {
	...
	ok, err := m.topologyLabelPresent(context.Background(), "gpu.deckhouse.io/pool-a", "pool-a", "")
	if err != nil || ok {
		t.Fatalf("expected empty topologyKey to return (false,nil), got (%v,%v)", ok, err)
	}

	ok, err = m.topologyLabelPresent(context.Background(), "gpu.deckhouse.io/pool-a", "pool-a", "topology.kubernetes.io/zone")
	if err != nil || !ok {
		t.Fatalf("expected topology label present, got (%v,%v)", ok, err)
	}

	ok, err = m.topologyLabelPresent(context.Background(), "gpu.deckhouse.io/pool-a", "pool-a", "topology.kubernetes.io/region")
	if err != nil || ok {
		t.Fatalf("expected topology label missing to return false, got (%v,%v)", ok, err)
	}

	// Empty node list -> ok=true (unknown yet).
	m = &podMutator{client: fake.NewClientBuilder().WithScheme(scheme).Build()}
	ok, err = m.topologyLabelPresent(context.Background(), "gpu.deckhouse.io/pool-a", "pool-a", "topology.kubernetes.io/zone")
	if err != nil || !ok {
	...
	m = &podMutator{client: &failingNodeListClient{Client: base, err: apierrors.NewBadRequest("list failed")}}
	ok, err = m.topologyLabelPresent(context.Background(), "gpu.deckhouse.io/pool-a", "pool-a", "topology.kubernetes.io/zone")
	if err == nil || ok {
...
func TestPodMutatorAllowsSpreadWithoutClientWhenNoConflict(t *testing.T) {
	...
	if len(pod.Spec.TopologySpreadConstraints) != 1 {
		t.Fatalf("expected spread constraint added, got %v", pod.Spec.TopologySpreadConstraints)
	}
	if pod.Spec.Tolerations[0].Operator != corev1.TolerationOpEqual && pod.Spec.Tolerations[0].Operator != corev1.TolerationOpExists {
		t.Fatalf("expected some toleration to be present")
	}
}
