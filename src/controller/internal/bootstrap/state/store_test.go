...
func TestSetOwnerReferenceIgnoresForbidden(t *testing.T) {
	base := newTestClient(t).Build()
	key := types.NamespacedName{Name: "controller", Namespace: "ns"}
	client := &errorClient{
		Client:  base,
		getErr:  apierrors.NewForbidden(schema.GroupResource{Group: "apps", Resource: "deployments"}, key.Name, errors.New("forbidden")),
		failKey: &key,
	}
	store := NewStore(client, client, "ns", "state", key)
	cm := &corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{Name: "state"}}
	if err := store.setOwnerReference(context.Background(), cm); err != nil {
		t.Fatalf("forbidden owner lookup should be ignored: %v", err)
	}
	if len(cm.OwnerReferences) != 0 {
		t.Fatalf("expected no owner references when forbidden, got %+v", cm.OwnerReferences)
	}
}
